...
class Slider:                        
    def __init__(self, x, y, w, h, max_value, col_slider, col_pointer):
    ...
    def draw(self, screen):
        ...
        # додай рамку
        pygame.draw.rect(screen, BLACK, self.slider, width=2, border_radius=5)
        ...
        pygame.draw.rect(screen, BLACK, self.pointer, width=2, border_radius=5)
        
        # обчисли позицію тексту по X : 
        #х.поінтера + ширина повзунка // 2.6
        x = self.pointer.x + self.pointer.width // 2.6
        # обчисли позицію тексту по Y : 
        #у.поінтера + ширина повзунка // 3
        y = self.pointer.y + self.pointer.height // 3
        # виведи текст значення на екран
        screen.blit(self.value_text, (x, y))
    ...
    def update_value(self):
        ...
        self.value = self.pointer.centerx - self.start
        # переведи  у значення: 
        # знайдене значення ділимо на ширину слайдера та множимо на максимальне значення слайдера
        #округлити значення
        self.value = int((self.value/self.slider.width) * self.max_num)
        # створити текст значення
        self.value_txt = self.font.render(str(int(self.value)), True, BLACK) 